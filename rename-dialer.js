/**
 * rename-dialer.js
 * 基于 Keywos/rule rename.js 修改，增加 dialer-proxy 引用同步更新功能
 *
 * 更新日期：2025-06-26
 * 用法：Sub-Store 脚本操作添加
 * 必须以 # 为开头，多个参数使用"&"连接，参考上述地址为例使用参数。 禁用缓存url#noCache
 *
 *** 新增参数（本脚本独有）
 * [rmdp]        当 dialer-proxy 指向的节点被过滤掉时，移除该 dialer-proxy 字段
 *               （不加此参数则保留原值，mihomo 会报错但你能发现哪些节点丢了）
 * [dpdebug]     开启 dialer-proxy 调试输出（控制台打印映射关系）
 *
 *** 主要参数
...
const FGF = inArg.fgf == undefined ? " " : decodeURI(inArg.fgf),
  ...
  // === 新增参数 ===
  RMDP = inArg.rmdp || false,
  DPDEBUG = inArg.dpdebug || false,
  nameMap = {
    cn: "cn",
    zh: "cn",
    us: "us",
...
// prettier-ignore
const QC = ['Hong Kong','Macao','Taiwan','Japan','Korea','Singapore','United States','United Kingdom','France','Germany','Australia','Dubai','Afghanistan','Albania','Algeria','Angola','Argentina','Armenia','Austria','Azerbaijan','Bahrain','Bangladesh','Belarus','Belgium','Belize','Benin','Bhutan','Bolivia','Bosnia and Herzegovina','Botswana','Brazil','British Virgin Islands','Brunei','Bulgaria','Burkina-faso','Burundi','Cambodia','Cameroon','Canada','CapeVerde','CaymanIslands','Central African Republic','Chad','Chile','Colombia','Comoros','Congo-Brazzaville','Congo-Kinshasa','CostaRica','Croatia','Cyprus','Czech Republic','Denmark','Djibouti','Dominican Republic','Ecuador','Egypt','EISalvador','Equatorial Guinea','Eritrea','Estonia','Ethiopia','Fiji','Finland','Gabon','Gambia','Georgia','Ghana','Greece','Greenland','Guatemala','Guinea','Guyana','Haiti','Honduras','Hungary','Iceland','India','Indonesia','Iran','Iraq','Ireland','Isle of Man','Israel','Italy','Ivory Coast','Jamaica','Jordan','Kazakstan','Kenya','Kuwait','Kyrgyzstan','Laos','Latvia','Lebanon','Lesotho','Liberia','Libya','Lithium','Luxembourg','Macedonia','Madagascar','Malawi','Malaysia','Maldives','Mali','Malta','Mauritania','Mauritius','Mexico','Moldova','Monaco','Mongolia','Montenegro','Morocco','Mozambique','Myanmar(Burma)','Namibia','Nepal','Netherlands','New Zealand','Nicaragua','Niger','Nigeria','NorthKorea','Norway','Oman','Pakistan','Panama','Paraguay','Peru','Philippines','Portugal','PuertoRico','Qatar','Romania','Russia','Rwanda','SanMarino','SaudiArabia','Senegal','Serbia','SierraLeone','Slovakia','Slovenia','Somalia','SouthAfrica','Spain','SriLanka','Sudan','Suriname','Swaziland','Sweden','Switzerland','Syria','Tajikstan','Tanzania','Thailand','Togo','Tonga','TrinidadandTobago','Tunisia','Turkey','Turkmenistan','U.S.Virgin Islands','Uganda','Ukraine','Uruguay','Uzbekistan','Venezuela','Vietnam','Yemen','Zambia','Zimbabwe','Andorra','Reunion','Poland','Guam','Vatican','Liechtensteins','Curacao','Seychelles','Antarctica','Gibraltar','Cuba','Faroe Islands','Ahvenanmaa','Bermuda','Timor-Leste'];
const specialRegex = [
  /(\d\.)?\d+×/,
...
const rurekey = {
  ...
  德国: /(深|沪|呼|京|广|杭)德(?!\s*(I|线))|法兰克福|滬德/g,
  香港: /(深|沪|呼|京|广|杭)港(?!\s*(I|线))/g,
  日本: /(深|沪|呼|京|广|杭|中|辽)日(?!\s*(I|线))|东京|大坂/g,
...
function operator(pro) {
  ...
  // ==================== 新增：更新 dialer-proxy 引用 ====================
  // 重命名完成后，构建 旧名称 → 新名称 的映射表，然后更新所有节点上的 dialer-proxy 字段
  // 这样 dialer-proxy 指向的节点名称会同步更新，避免 mihomo 报 "dialer-proxy not found"
  ...
  if (DPDEBUG) {
    console.log("=== dialer-proxy 映射调试 ===");
    console.log("旧名 → 新名 映射表:");
    Object.keys(renameMap).forEach((old) => {
      console.log(`  "${old}" → [${renameMap[old].map((n) => `"${n}"`).join(", ")}]`);
    });
  }

  // 2. 遍历所有节点，更新 dialer-proxy 引用
  let dpUpdated = 0,
    dpNotFound = 0,
    dpAmbiguous = 0;
  pro.forEach((e) => {
    const dp = e["dialer-proxy"];
    if (dp != null && typeof dp === "string") {
      const mapped = renameMap[dp];
      if (mapped) {
        if (mapped.length === 1) {
          // 唯一匹配，直接更新
          if (dp !== mapped[0]) {
            if (DPDEBUG) console.log(`更新 dialer-proxy: "${dp}" → "${mapped[0]}" (节点: ${e.name})`);
            e["dialer-proxy"] = mapped[0];
            dpUpdated++;
          }
        } else {
          // 多个节点同名，取第一个并警告
          dpAmbiguous++;
          if (DPDEBUG) {
            console.log(`⚠️ 节点 "${e.name}" 的 dialer-proxy "${dp}" 匹配到多个节点: [${mapped.map((n) => `"${n}"`).join(", ")}]，使用第一个`);
          }
          e["dialer-proxy"] = mapped[0];
        }
      } else {
        // 目标节点不在列表中（可能被过滤掉了，或者是不在此订阅中的外部节点）
        dpNotFound++;
        if (DPDEBUG) {
          console.log(`❌ 节点 "${e.name}" 的 dialer-proxy "${dp}" 未找到对应节点`);
        }
        if (RMDP) {
          delete e["dialer-proxy"];
          if (DPDEBUG) console.log(`   已移除 dialer-proxy (rmdp 模式)`);
        }
      }
    }
    // 清理临时字段
  ...
  if (DPDEBUG) {
    console.log(`=== dialer-proxy 统计: 更新 ${dpUpdated}, 未找到 ${dpNotFound}, 多义 ${dpAmbiguous} ===`);
  }
  // ==================================================================
